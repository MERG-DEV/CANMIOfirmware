...
# 53 "../../github/CBUSlib/timedResponse.h"
...
# 230
const EventTable * eventTable =(EventTable *)0xEF80U;

# 255
BYTE happening2Event[(8U + 16U * 4U)];

# 263
BYTE eventChains[32U][20U];
...
cbusMsg[d3] = count;
cbusSendOpcMyNN( 0, 0x70, cbusMsg );
}
...
cbusMsg[d7] = index;
cbusSendOpcMyNN( 0, 0xF2, cbusMsg );
...
cbusMsg[d3] = count;
cbusSendOpcMyNN( 0, 0x74, cbusMsg );
}
...
for (e = 0; e < 10U; e++) {
writeFlashByte((BYTE*)&(eventTable[tableIndex].evs[e]), 0);
}
flushFlashImage();
...
for (chainIdx=0; chainIdx<20U; chainIdx++) {
...
for (nextIdx = tableIndex+1U ; nextIdx < 255U; nextIdx++) {
EventTableFlags nextF;
nextF.asByte = readFlashBlock((WORD)(&(eventTable[nextIdx].flags.asByte)));
if (nextF.freeEntry) {
unsigned char e;

setFlashWord((WORD*)&(eventTable[nextIdx].event.NN), 0xffff);
setFlashWord((WORD*)&(eventTable[nextIdx].event.EN), 0xffff);
writeFlashByte((BYTE*)&(eventTable[nextIdx].flags.asByte), 0x20);
for (e = 0; e < 10U; e++) {
writeFlashByte((BYTE*)&(eventTable[nextIdx].evs[e]), 0);
}
...
for (evNum=0; evNum < 20U; ) {
unsigned char evIdx;
for (evIdx=0; evIdx < 10U; evIdx++) {
...
for (happening=0; happening<(8U + 16U * 4U); happening++) {
happening2Event[happening] = 0xff;
}

for (hash=0; hash<32U; hash++) {
for (chainIdx=0; chainIdx < 20U; chainIdx++) {
...
if (a >= 0) {
happening = (unsigned char)a;
if ((happening >= 1U) && (happening-1U< (8U + 16U * 4U))) {
happening2Event[happening-1U] = tableIndex;
}
}
