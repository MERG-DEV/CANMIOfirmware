...
# 82
enum FLiMStates flimState;
enum FLiMStates prevFlimState;
TickValue switchTime;




extern const NodeVarTable nodeVarTable @0xFF80U ;
const BYTE * NvBytePtr = nodeVarTable.nodevars;
...
flimState = ee_read((WORD)((BYTE*)(0x3FF-4)));
...
if (!PORTAbits.RA2)
{
if ((tickGet() - switchTime.Val) > 4 * 62500)
{
flimState = fsFlashing;
startFLiMFlash(0);
}
}
else if ((prevFlimState == fsFLiM) && ((tickGet() - switchTime.Val) > (62500/10)))
{
...
if (PORTAbits.RA2)
{
if (((tickGet() - switchTime.Val) > (62500/10)))
{
flimState = prevFlimState;
setStatusLed(flimState == fsFLiM);
}
...
cbusSendOpcNN(0xFF, 0x50, -1, local_cbusMsg);
...
cbusSendOpcNN(0xFF, 0x51, -1, cbusMsg);
...
if (((msg[d0] & 0b10010000) == 0b10010000) && ((~msg[d0] & 0b00000110)== 0b00000110))
{
...
doEvlrn((((WORD)rx_ptr[d1]) << 8) + rx_ptr[d2] , (((WORD)rx_ptr[d3]) << 8) + rx_ptr[d4], rx_ptr[d6], rx_ptr[d7]);
break;
...
if (flimState == fsFLiMLearn) {
return(32U | 4U) | (paramptr->params.module_flags);
}
if (flimState == fsFLiM) {
return(4U)|(paramptr->params.module_flags);
}
return 0;
}
...
cbusMsg[d4] = (unsigned char)(readCPUType() >> ((unsigned char)( idx - 15 )*8U));
else
cbusMsg[d4] = paramptr->bytes[idx-1U];

if ((idx == 8) && (flimState == fsFLiM))
cbusMsg[d4] |= 4U;
...
if ((NVindex == 0) || (NVindex >= sizeof(ModuleNvDefs)))
{
doError(10);
}
else
...
cbusSendOpcMyNN( 0, 0x59, cbusMsg);
}
else
{
doError(12);
...
cbusMsg[(unsigned char)d1+8-1U] = getParFlags();
...
cbusSendOpcMyNN( 0, 0x52, cbusMsg );
setFLiMLed();
...
{
ee_write_short((WORD)((WORD*)(0x3FF-3)), nodeID);
ee_write((WORD)((BYTE*)(0x3FF-4)), flimState);
...
evNum--;
error = APP_addEvent(nodeNumber, eventNumber, evNum, evVal, 0);
if (error)
{
...
cbusSendOpcMyNN( 0, 0xB5, cbusMsg );
return;
}
cbusMsg[d3] = (unsigned char)(-evVal);
cbusSendOpcMyNN( 0, 0x6F, cbusMsg);
return;
}
cbusMsg[d3] = 7;
...
cbusSendOpcMyNN( 0, 0xd3, cbusMsg);
return;
}
cbusMsg[d3] = (unsigned char)(-evVal);
cbusSendOpcMyNN( 0, 0x6F, cbusMsg);
