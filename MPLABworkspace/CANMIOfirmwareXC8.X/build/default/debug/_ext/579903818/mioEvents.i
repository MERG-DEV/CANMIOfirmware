...
# 1 "../../github/CANMIOfirmware/mioEvents.c"
...
# 70 "../../github/CANMIOfirmware/mioEvents.c"
...
BOOL sendInvertedProducedEvent(unsigned char happening, BOOL state, BOOL invert, BOOL can_send_on, BOOL can_send_off);
...
case 3U:
addEvent(nodeID, 100+en, 0, ((8U + 4U*(io))+0U), 1);


case 1U:

addEvent(nodeID, en, 1, ((8U + 5U*(io))+0U), 1);


break;
case 0U:

addEvent(nodeID, en, 0, ((8U + 4U*(io))+0U), 1);
break;

case 2U:

addEvent(nodeID, 100+en, 0, ((8U + 4U*(io))+0U), 1);
addEvent(nodeID, 300+en, 0, ((8U + 4U*(io))+1U), 1);
addEvent(nodeID, 200+en, 0, ((8U + 4U*(io))+2U), 1);
...
addEvent(nodeID, en, 0, ((8U + 4U*(io))+1U), 1);
addEvent(nodeID, 100+en, 0, ((8U + 4U*(io))+0U), 1);
break;
...
unsigned char tableIndex = happening2Event[evVal-1U];
if (tableIndex != 0xff) {
...
# 189
BOOL getDefaultProducedEvent(unsigned char paction) {
return 0;
}

# 198
void clearEvents(unsigned char io) {
deleteActionRange((8U + 5U*(io)), 5U);
deleteActionRange((8U + 5U*(io)) | 0x80U, 5U);
deleteHappeningRange((8U + 4U*(io)), 4U);
}
...
masked_action = (unsigned char)action&0x7FU;
if ((masked_action) <= (8U + 16U * 5U)) {

if ((masked_action) == 7U) {
break;
}
if ((masked_action) < 8U) {
pushAction((unsigned char)action);
} else {
io = (((masked_action)-8U)/5U);
ca = (((masked_action)-8U)%5U);
switch (NV->io[io].type) {
case 1U:
if (NV->io[io].flags & 0x80U) {
setExpeditedActions();
}
...
int nextAction = evs[20U-1];
for (e=20U-1; e>=1 ;e--) {
unsigned char nextSimultaneous;
unsigned char firstAction = 0;
action = nextAction;
...
if (e > 1) {
nextAction = evs[e-1U];
nextSimultaneous = (unsigned char)nextAction & 0x80U;
} else {
nextSimultaneous = (unsigned char)firstAction & 0x80U;
}
if (action != 0) {
...
action &= 0x7FU;
if ((unsigned char)action <= (8U + 16U * 5U)) {

if ((unsigned char)action == 7U) {
break;
}
if (((unsigned char)action < 8U) && ((unsigned char)action != 1U)) {
pushAction((unsigned char)((unsigned char)action|nextSimultaneous));
} else {
io = ((((unsigned char)action)-8U)/5U);
ca = ((((unsigned char)action)-8U)%5U);
switch (NV->io[io].type) {
case 1U:
if (NV->io[io].flags & 0x80U) {
setExpeditedActions();
...
simultaneous = action & 0x80U;
ioAction = action&0x7FU;
if ((ioAction >= 8U) && (ioAction < (8U + 16U * 5U))) {


io = (((ioAction)-8U)/5U);
ioAction = (((ioAction)-8U)%5U);
type = NV->io[io].type;
...
simultaneous = nextAction & 0x80U;
nextAction &= 0x7FU;
nextIo = (((nextAction)-8U)/5U);
if (nextIo == io) {
...
if (((tickGet() - startWait.Val) > ((unsigned long)duration * (unsigned long)(62500/10)))) {
doneAction();
startWait.Val = 0;
return;
}
...
happeningIndex = step%4U;
flags = NV->io[io].flags;
disable_SOD_response = flags & 0x20U;

event_inverted = flags & 0x40U;
disable_off = flags & 0x08U;
...
switch (happeningIndex) {
case 0U:
value = (unsigned char)(ee_read(((WORD)(((BYTE*)(0x3FF-40)))-17)+io)!=2U);
break;
}
break;
...
switch (happeningIndex) {
case 0U:
value = ee_read(((WORD)(((BYTE*)(0x3FF-40)))-17)+io);
break;
}
send_on_ok = 1;
send_off_ok = 1;
...
switch (happeningIndex) {
case 0U:
value = (unsigned char)(analogueState[io].eventState == 1U);
break;
case 1U:
value = (unsigned char)(analogueState[io].eventState == 2U);
break;
}
break;
...
if (value != 255) {
if (!sendInvertedProducedEvent((8U + 4U*(io))+happeningIndex, value, event_inverted, send_on_ok, send_off_ok)) {
return 1;
}
}
return 2;
